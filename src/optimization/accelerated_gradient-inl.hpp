...
inline solver_return accelerated_gradient(
    first_order_opt_interface& model, const DenseVector& init_point,
  ...
  std::vector<std::string> stat_info = {
      std::to_string(iters), std::to_string(stats.num_passes),
      std::to_string(step_size), std::to_string(tmr.current_time())};
  ...
  while ((residual >= convergence_threshold) && (iters < iter_limit)) {
    ...
    while (step_size > LS_ZERO) {
      ...
      if (reg != NULL) reg->apply_proximal_operator(point, step_size);
      ...
      delta_point = (point - y);
      Qply = fy + dot(delta_point, gradient) +
    ...
    // Print progress
    stat_info = {std::to_string(iters), std::to_string(stats.num_passes),
                 std::to_string(step_size), std::to_string(tmr.current_time())};
