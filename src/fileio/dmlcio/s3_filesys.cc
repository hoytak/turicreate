...
size_t CURLReadStreamBase::Read(void *ptr, size_t size) {
  ...
  while(nleft != 0) {
    ...
    // retry
    if (retry_count < 5 && nleft > 0 &&
        at_end_ && expect_file_size_ != 0 &&
...
void ReadStream::InitRequest(size_t begin_bytes,
  ...
  sdate << "Date: " << date;
  surl << get_bucket_path(path_.host)
...
void WriteStream::Run(const std::string &method,
  ...
  std::string date = GetDateString();
  auto lowerhost = boost::algorithm::to_lower_copy(path.host);
  ...
  sauth << "Authorization: AWS " << aws_id_ << ":" << signature;
  sdate << "Date: " << date;
  surl << get_bucket_path(path.host)
       << RemoveBeginSlash(path_.name) << args;
...
void WriteStream::Upload(bool force_upload_even_if_zero_bytes) {
  ...
  const char *p = strcasestr(rheader.c_str(), "ETag: ");
  ASSERT_MSG((p != NULL), "cannot find ETag in header");
  p += 6;
  const char *end = strchr(p + 1, '\n');
  auto etag = std::string(p, end - p + 1);
  etag = boost::algorithm::trim_copy_if(etag, boost::algorithm::is_any_of(" \"\r\n"));
