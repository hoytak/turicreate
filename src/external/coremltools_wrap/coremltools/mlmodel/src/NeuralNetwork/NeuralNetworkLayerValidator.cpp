...
Result NeuralNetworkSpecValidator::validateCopyLayer(const Specification::NeuralNetworkLayer& layer) {
    ...
    if (layer.input(0) == layer.output(0)) {
        std::string err = "Copy layer '" + layer.name() + "' has identical input and output names.";
        return Result(ResultType::INVALID_MODEL_PARAMETERS, err);
    }
    return r;
}
...
Result NeuralNetworkSpecValidator::validateBooleanElementwiseLayer(const Specification::NeuralNetworkLayer& layer) {
    ...
    if (layer.inputtensor_size() > 1) {
        int rank1 = static_cast<int>(layer.inputtensor(0).rank());
        int rank2 = static_cast<int>(layer.inputtensor(1).rank());
        if (rank1 != rank2) {
            std::string err = "Elementwise boolean layer: '" + std::string(layer.name()) + "' has inputs whose ranks do not match'.";
            return Result(ResultType::INVALID_MODEL_PARAMETERS, err);
        }
    }
    return r;
...
//    To do: Update validator for newly added brick layers: rdar://problem/45861295
Result NeuralNetworkSpecValidator::validateStackNDLayer(const Specification::NeuralNetworkLayer& layer) {
    Result r;
    r = validateInputCount(layer, 2, -1);
    if (r.good()) {
        r = validateOutputCount(layer, 1, 1);
...
//    To do: Update validator for newly added brick layers: rdar://problem/45861295
Result NeuralNetworkSpecValidator::validateSliceNDLayer(const Specification::NeuralNetworkLayer& layer) {
    Result r;
    r = validateInputCount(layer, 1, 1);
    if (r.good()) {
        r = validateOutputCount(layer, 1, 1);
...
Result NeuralNetworkSpecValidator::validateRangeLayer(const Specification::NeuralNetworkLayer& layer) {
    Result r;
