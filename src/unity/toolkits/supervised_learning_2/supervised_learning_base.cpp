...
void supervised_learning_model_base::setup_options() {

  options.create_flexible_type_option("features",
                                      "List of features that are considered "
                                      "during training and prediction for this "
                                      "model.",
                                      FLEX_UNDEFINED, false);

  options.create_string_option("target", "Target column for training.",
                               FLEX_UNDEFINED);

  if (feature_rescaling_allowed()) {
    options.create_boolean_option("feature_rescaling",
                                  "Rescale features to have unit L2-Norm.", true,
                                  false);
  }

  if (is_classifier()) {
    options.create_flexible_type_option(
        "class_weights", "Weights (during training) assigned to each class.",
        FLEX_UNDEFINED, false);
  }
...
void supervised_learning_model_base::setup_iterative_training(
  ...
  /////////////////////////////////////////////////////////////////////////////
  // If it's a classifier, test to make sure that things are solid.
    ...
    for (size_t cid = 0; cid < metadata->num_columns(); cid++) {
      ...
      for (size_t i = 0; i < index_size; i++) {
        if (std::abs(stats->stdev(i)) < 1e-20) {
          error_columns.push_back(metadata->column_name(cid));
          break;
        }
      }
    }

    if (!error_columns.empty() && m_display_warnings) {
  ...
  /////////////////////////////////////////////////////////////////////////////
  // Dump a lot of values into the state to store them.
...
/**
 * Sets up the class weights based on the user options and target metadata.
 *
 * \param[in] options
 * \param[in] metadata
  ...
  // Case 2 ('auto'): Sample inversely proportional to class frequency.
    ...
    for(const auto& kvp : class_weights_option.get<flex_dict>()) {
      ...
      if ((kvp.second.get_type() == flex_type_enum::INTEGER
          ...
          && (std::is_finite(kvp.second.to<double>()))
          && (kvp.second.to<double>() > 1e-20)) {

        size_t index = indexer->map_value_to_index(p.irst);

        if (index >= num_classes) {
          classes_not_mapped.push_back(kvp.first);
          continue;
        }

        m_class_weights[index] = p.second;

      } else {
        // It's not a valid weight
        std::ostringstream ss;

        ss << "The class_weight parameter for the class " << kvp.first
           << " must be a positive numeric value." << std::endl;

        log_and_throw(ss.str());
      }
    }

    if (!classes_not_mapped.empty()) {
      std::ostringstream ss;

      ss << "Warning: The following classes given as class weights were not "
         << "present in the training data: ";

      for (size_t i = 0; i < classes_not_mapped.size(); ++i) {
        ss << classes_not_mapped[i]
           << ((i == classes_not_mapped.size() - 1) ? "." : ", ");
      }
    ...
    // Now, error out if any were not set.
    {
      std::vector<size_t> nan_indices;

      for (size_t i = 0; i < num_classes; i++) {
        if (std::is_nan(class_weights[i])) {
          nan_indices.push_back(i);
        }
      }

      if (!nan_weights.empty()) {
        std::ostringstream ss;

        ss << "The weights for the following classes were not specified: ";

        for (size_t i = 0; i < nan_weights.size(); ++i) {
          ss << indexer->map_index_to_value(nan_weights[i])
             << ((i == nan_weights.size() - 1) ? "." : ", ");
        }
