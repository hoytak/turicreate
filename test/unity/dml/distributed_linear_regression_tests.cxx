...
*/
struct distributed_linear_regression_test  {
 ...
 public:
  ...
  void test_impl(std::map<std::string, flexible_type> opts,
    ...
    try {
      ...
      params["model_name"] = std::string("regression_linear_regression");
      ...
      std::shared_ptr<linear_regression> model =
            variant_get_value<std::shared_ptr<linear_regression>>(ret);
...
BOOST_FIXTURE_TEST_SUITE(_distributed_linear_regression_test, distributed_linear_regression_test)
BOOST_AUTO_TEST_CASE(test_linear_regression_basic_2d) {
  distributed_linear_regression_test::test_linear_regression_basic_2d();
}
BOOST_AUTO_TEST_CASE(test_linear_regression_small) {
  distributed_linear_regression_test::test_linear_regression_small();
}
